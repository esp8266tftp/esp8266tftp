...
$(TARGET_OUT): $(APP_AR)
	...
	$(vecho) "Done"
	$(SDK_TOOLS)/MemAnalyzer.exe $(XTENSA_TOOLS_ROOT)/xtensa-lx106-elf-objdump.exe $(TARGET_OUT)
...
flashonefile: all
	...
	$(vecho) "Generate eagle.app.flash.bin successully__ in folder $(FW_BASE)."
...
flashapp: all
	$(ESPTOOL) -p $(ESPPORT) -b $(BAUD) write_flash $(flashimageoptions) 0x00000 $(FW_BASE)/0x00000.bin
